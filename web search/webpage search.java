...
The URL must be complete and exact including HTTP:// HTTPS:// (copy from browser address). The search string must be unique and exact including case, spaces, special characters, etc. (copy from site’s webpage loaded in browser). Content created by webpage dynamic scripts will probably not be found by the driver. Create a virtual device from the driver to use in a tile.
*/
